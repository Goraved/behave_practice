...
mkdir -p allure-results && mkdir -p allure-results/archive && mkdir -p allure-results/history

# Run The tests in project folder
behave -f allure_behave.formatter:AllureFormatter -o allure-results/archive/${_now} ./features
...
rm ${dir}/allure-results/history/*.json
